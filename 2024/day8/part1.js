    ...
    for (let i = 0; i < array.length; i++) {
        ...
        for (let node of otherNodes) {
            ...
            if (extendNodes(array[i], heightAway, widthAway, height, width)) {
                foundAntinodes.push(`${array[i][0] + (heightAway)},${array[i][1] + (widthAway)}`);
            }
...
function extendNodes(currentNode, heightMovement, widthMovement, height, width) {
    let movedNode = [(currentNode[0] + (heightMovement)),(currentNode[1] + (widthMovement))]
    if (movedNode[0] < height &&
        movedNode[0] >= 0 &&
        movedNode[1] < width &&
        movedNode[1] >= 0) {
        return true;
