...
function iterateResults(expecResult, currentResult, integers, index) {
  ...
  if (
    additionResult == expecResult ||
    multResult == expecResult ||
    concatResult == expecResult
  ) {
    return true;
  }
  ...
  if (index < integers.length - 1) {
    ...
    if (
      additionResult < expecResult &&
      iterateResults(expecResult, additionResult, integers, index)
    ) {
      return true;
    } else if (
      multResult < expecResult &&
      iterateResults(expecResult, multResult, integers, index)
    ) {
      return true;
    } else if (
      concatResult < expecResult &&
      iterateResults(expecResult, concatResult, integers, index)
    ) {
      return true;
    } else {
      return false;
